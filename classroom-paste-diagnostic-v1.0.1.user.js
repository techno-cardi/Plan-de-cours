...
// @version      1.0.1
// @description  Prépare le plan riche puis observe un collage manuel réussi dans une annonce Classroom afin de produire un rapport précis.
// @author       techno-cardi
...
(function () {
  ...
  if (isPlanPage) {
    ...
    const title = make('div', 'Diagnostic Classroom');
    title.style.cssText = 'font-weight:700;font-size:14px;margin-bottom:6px';
    const info = make('div', '1. Remplis ton plan normalement. 2. Clique le bouton ci-dessous. 3. Va dans Classroom, ouvre « Nouvelle annonce » et fais Ctrl+V manuellement.');
    info.style.cssText = 'color:#d2d5d9;margin-bottom:10px';
    const btn = make('button', 'Préparer le collage diagnostic');
    btn.style.cssText = 'border:0;border-radius:7px;padding:8px 10px;cursor:pointer;font:700 12px Arial,sans-serif';
    btn.onclick = async () => {
      btn.disabled = true;
      ...
      try {
        ...
        const fn = unsafeWindow.generer || window.generer;
        if (typeof fn !== 'function') throw new Error('fonction generer introuvable');
        await fn();
        await new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve)));
        const preview = document.getElementById('plan-preview');
        const html = preview?.innerHTML?.trim() || '';
        const text = preview?.innerText?.trim() || '';
        if (!html || !text || preview.querySelector('.empty-state')) throw new Error('aperçu vide');
        GM_setClipboard(html, 'html');
        btn.textContent = 'Plan riche copié - va dans Classroom';
        info.textContent = 'Le plan riche exact est dans le presse-papier. Dans Classroom: Nouvelle annonce -> clique dans le champ -> Ctrl+V. Ensuite clique « Copier le rapport » dans le panneau diagnostic.';
      } catch (err) {
        console.error('[Diagnostic Classroom]', err);
        btn.textContent = 'Échec de préparation';
        info.textContent = 'Le plan n’a pas pu être préparé: ' + (err?.message || err);
      } finally {
  ...
  function findAnnouncementEditor() {
    ...
    for (const el of all) {
      ...
      const txt = norm([
        ...
        el.parentElement?.parentElement?.parentElement?.textContent
      ].join(' ')).toLowerCase();
      let score = 0;
  ...
  function snapshot(label, editor = findAnnouncementEditor()) {
    ...
    add('snapshot', {
      ...
      editorInnerText: short(editor?.innerText || editor?.textContent || '', 16000),
      editorInnerHTML: short(editor?.innerHTML || '', 26000),
      editorOuterHTML: short(editor?.outerHTML || '', 32000),
      publishFound: !!button,
  ...
  document.addEventListener('keydown', e => {
    ...
    if ((e.ctrlKey || e.metaKey) && String(e.key).toLowerCase() === 'v') {
      add('keydown-paste', { target: cssPath(e.target), key: e.key, ctrlKey: e.ctrlKey, metaKey: e.metaKey, isTrusted: e.isTrusted });
    }
  }, true);
  ...
  document.addEventListener('beforeinput', e => {
    if (!isRelevantTarget(e.target)) return;
    add('beforeinput', {
      target: cssPath(e.target), inputType: e.inputType, data: e.data, isTrusted: e.isTrusted,
  ...
  document.addEventListener('input', e => {
    ...
    add('input', {
      target: cssPath(e.target), inputType: e.inputType, data: e.data, isTrusted: e.isTrusted,
      textAfter: short(e.target.innerText || e.target.textContent || '', 7000),
      htmlAfter: short(e.target.innerHTML || '', 14000)
    });
  }, true);
  ...
  document.addEventListener('paste', e => {
    ...
    add('paste', {
      target: cssPath(e.target), activeElement: cssPath(document.activeElement), isTrusted: e.isTrusted,
      cancelable: e.cancelable, defaultPrevented: e.defaultPrevented,
      clipboardTypes: types, clipboardPlain: short(plain, 24000), clipboardHtml: short(html, 36000),
      selection: selectionSnapshot()
    });
    snapshot('juste avant traitement du collage', editorAtPaste);
    setTimeout(() => snapshot('après collage + 0 ms', editorAtPaste), 0);
    setTimeout(() => snapshot('après collage + 25 ms', editorAtPaste), 25);
    setTimeout(() => snapshot('après collage + 100 ms', editorAtPaste), 100);
    setTimeout(() => snapshot('après collage + 300 ms', editorAtPaste), 300);
  ...
  const observer = new MutationObserver(records => {
    ...
    for (const r of records) {
      ...
      relevant.push({
        ...
        added: Array.from(r.addedNodes || []).slice(0, 6).map(n => short(n.outerHTML || n.textContent || n.nodeName, 4000)),
        removed: Array.from(r.removedNodes || []).slice(0, 6).map(n => short(n.outerHTML || n.textContent || n.nodeName, 4000))
      });
    }
  ...
  function makeReport() {
    ...
    return JSON.stringify({
      diagnosticVersion: '1.0.1', url: location.href, userAgent: navigator.userAgent,
      timestamp: new Date().toISOString(), pasteSeen, mutationCount, logs
    }, null, 2);
  }
  ...
  function createPanel() {
    ...
    box.style.cssText = 'position:fixed;right:14px;bottom:14px;z-index:2147483647;width:min(410px,calc(100vw - 28px));background:#202124;color:#fff;border-radius:12px;padding:14px 15px;box-shadow:0 8px 30px rgba(0,0,0,.35);font:13px/1.42 Arial,sans-serif';
    const title = document.createElement('div');
    title.style.cssText = 'font-weight:700;font-size:14px;margin-bottom:6px';
    title.textContent = 'Diagnostic collage Classroom v1.0.1';
    const info = document.createElement('div');
    info.id = 'pdc-diag-info';
    info.style.cssText = 'color:#d2d5d9;margin-bottom:10px';
    const actions = document.createElement('div');
    actions.style.cssText = 'display:flex;gap:7px;flex-wrap:wrap';
    const snap = document.createElement('button'); snap.textContent = 'Capturer état';
    const copy = document.createElement('button'); copy.textContent = 'Copier le rapport';
    const reset = document.createElement('button'); reset.textContent = 'Recommencer';
    const hide = document.createElement('button'); hide.textContent = 'Masquer';
    for (const b of [snap, copy, reset, hide]) b.style.cssText = 'border:0;border-radius:7px;padding:7px 9px;cursor:pointer;font:600 12px Arial,sans-serif';
    ...
    reset.onclick = () => {
      logs.length = 0; pasteSeen = false; editorAtPaste = null; mutationCount = 0; add('reset', {});
    };
  ...
  function updatePanel() {
    ...
    info.textContent = pasteSeen
      ? `Collage détecté. ${logs.length} événements, ${mutationCount} mutations. Attends 3 secondes, puis clique « Copier le rapport ».`
      : `Ouvre « Nouvelle annonce », clique dans le vrai champ, puis Ctrl+V. Éditeur: ${editor ? 'trouvé' : 'pas encore'} | Publier: ${pub ? 'trouvé' : 'pas encore'}.`;
  }
