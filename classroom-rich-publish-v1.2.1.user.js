  ...
  function installGeneratorBridge() {
    ...
    document.addEventListener('pdc:publish-course', event => {
      ...
      function onAck(message) {
        ...
        handling = false;
        if (!message.data.ok) {
          document.dispatchEvent(new CustomEvent('pdc:publish-result', { detail: { requestId, outcome: 'failed', error: message.data.error || 'pont Chrome indisponible' } }));
          alert(`Publication Classroom impossible : ${message.data.error || 'pont Chrome indisponible'}`);
        }
      }
      ...
      try {
        ...
        timer = setTimeout(() => {
          ...
          window.removeEventListener('message', onAck);
          handling = false;
          document.dispatchEvent(new CustomEvent('pdc:publish-result', { detail: { requestId, outcome: 'failed', error: 'pont Chrome natif non détecté' } }));
