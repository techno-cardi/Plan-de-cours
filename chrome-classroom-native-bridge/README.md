# Pont natif Classroom v1.0.7

Ce module Chrome accompagne le userscript `classroom-rich-publish.user.js` à partir de la version 1.2.2. La v1.0.7 active immédiatement l’onglet Classroom, reconnaît le bouton d’annonce dans les différentes dispositions responsives de Classroom, attend le chargement lent de l’interface, termine le flux, revient au générateur et ferme automatiquement l’onglet technique. L’accusé de réception et le résultat vérifié passent aussi par le DOM afin de rester fiables malgré l’isolation de Tampermonkey.
