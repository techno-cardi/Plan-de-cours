# Pont natif Classroom v1.0.3

Ce module Chrome accompagne le userscript `classroom-rich-publish.user.js` à partir de la version 1.2.1. La v1.0.3 tente d’abord la publication dans un onglet d’arrière-plan. Si Classroom suspend son interface, le pont active temporairement cet onglet, termine le flux, revient au générateur et ferme automatiquement l’onglet technique. Le résultat vérifié est conservé jusqu’à sa réception par le générateur.
