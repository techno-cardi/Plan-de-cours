...
(function () {
  ...
  function cleanRichHtml(html) {
    ...
    const allowed = new Set(['P', 'DIV', 'BR', 'B', 'STRONG', 'I', 'EM', 'U', 'S', 'STRIKE']);

    function clean(node) {
      if (node.nodeType === Node.TEXT_NODE) return document.createTextNode(node.nodeValue || '');
      if (node.nodeType !== Node.ELEMENT_NODE) return document.createDocumentFragment();
      const emojiFallback = node.getAttribute?.('alt') || node.getAttribute?.('data-emoji') || node.getAttribute?.('aria-label') || '';
      if (!String(node.textContent || '').trim() && emojiFallback && (node.tagName === 'IMG' || /[\u2600-\u27BF]|[\u{1F300}-\u{1FAFF}]/u.test(emojiFallback))) {
        return document.createTextNode(emojiFallback);
      }
      const tag = allowed.has(node.tagName) ? node.tagName.toLowerCase() : null;
      const outputTag = tag === 'strong' ? 'b' : tag === 'em' ? 'i' : tag === 'strike' ? 's' : tag;
      const output = outputTag ? document.createElement(outputTag) : document.createDocumentFragment();
      Array.from(node.childNodes).forEach(child => output.appendChild(clean(child)));
      return output;
    }

    const result = document.createElement('div');
    Array.from(source.childNodes).forEach(child => result.appendChild(clean(child)));
  ...
  function richHtmlToClassroomText(html) {
    ...
    function walk(node) {
      ...
      if (tag === 'br') return '\n';
      let text = '';
      Array.from(node.childNodes).forEach(child => { text += walk(child); });
      return tag === 'p' || tag === 'div' ? `${text}\n\n` : text;
    }

    let text = '';
    Array.from(source.childNodes).forEach(child => { text += walk(child); });
    return text
      .replace(/\u00a0/g, ' ')
      .replace(/[ \t]+\n/g, '\n')
      .replace(/\n{3,}/g, '\n\n')
